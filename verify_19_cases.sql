...
WHERE table_schema = 'public' 
  AND table_name IN ('court_cases', 'case_factors', 'case_holdings', 'case_analysis_metadata')
ORDER BY table_name;
...
SELECT 
    ...
    cam.is_analyzed,
    (SELECT COUNT(*) FROM public.case_factors WHERE case_id = cc.id) AS factor_count,
    (SELECT COUNT(*) FROM public.case_holdings WHERE case_id = cc.id) AS holding_count
FROM public.court_cases cc
LEFT JOIN public.case_analysis_metadata cam ON cam.case_id = cc.id
...
SELECT 
    ...
    SUM((SELECT COUNT(*) FROM public.case_factors WHERE case_id = CC.id)) AS total_factors,
    SUM((SELECT COUNT(*) FROM public.case_holdings WHERE case_id = CC.id)) AS total_holdings
FROM public.court_cases CC
LEFT JOIN public.case_analysis_metadata CAM ON CAM.case_id = CC.id
WHERE CC.id BETWEEN 537 AND 555
   OR (CC.source = 'user_input' AND CC.id >= 537);
