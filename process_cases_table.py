...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("case_processing.log"),
...
def map_case_schema(source_case: Dict) -> Dict:
    ...
    court_name = source_case.get("court") or ""
    court_type = infer_court_type(court_name)
    
    # Parse date
    date_filed = source_case.get("date_filed")
    decision_date = None
    if date_filed:
        if isinstance(date_filed, str):
            try:
                decision_date = datetime.fromisoformat(date_filed.replace('Z', '+00:00')).date()
            except:
                try:
                    decision_date = datetime.strptime(date_filed.split()[0], '%Y-%m-%d').date()
                except:
                    pass
        elif hasattr(date_filed, 'date'):
            decision_date = date_filed.date()
    
    # Use case id as docket_number if no citation
    docket_number = source_case.get("citation") or source_case.get("id")
    
    mapped_case = {
        "case_name": source_case.get("case_name", "Unknown Case"),
        "opinion_text": source_case.get("full_text") or source_case.get("summary") or "",
        ...
        "opinion_url": source_case.get("url"),
        "source": "cases_table_migration",
        "is_published": True,
        "is_downloaded": True,
    }
...
def process_single_case(
    ...
    try:
        ...
        if analysis.get("factors"):
            ...
            try:
                client.table("case_factors").delete().eq("case_id", court_case_id).execute()
            except:
                pass
            ...
            if factors_data:
                try:
                    ...
                    logger.debug(f"Error inserting factors for case {court_case_id}: {e}")
        ...
        if citations:
            ...
            for citation in citations:
                ...
                if citation_text and len(citation_text) > 5:
                    try:
                        cited_case_id = citation_extractor.match_citation_to_case(citation_text)
                    except:
                        cited_case_id = None
...
def process_cases_batch(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_single_case, case, analyzer, citation_extractor, client, force): case
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if success:
                    ...
                    batch_error_count += 1
                    if error:
                        logger.debug(f"Error processing case {case.get('id')}: {error[:100]}")
            except Exception as e:
                batch_error_count += 1
...
def process_all_cases(
    ...
    logger.info(f"Batch size: {batch_size}, Analysis workers: {max_workers}, Force: {force}")
    ...
    if failed_cases:
        ...
        for case_id, case_name, error_msg in failed_cases:
            logger.error(f"Case ID: {case_id}, Name: {case_name[:100]}, Error: {error_msg[:200]}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-analyze cases even if already analyzed"
    )
    parser.add_argument(
        "--limit",
        type=int,
        help="Limit total number of cases to process (for testing)"
    )
    parser.add_argument(
        "--resume-from",
        type=str,
